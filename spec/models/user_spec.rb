...
RSpec.describe User, type: :model do
  ...
  describe "ユーザー新規登録" do
    ...
    it "6passwordが空では登録できない" do
      @user.password = nil
      @user.valid?
      expect(@user.errors.full_messages).to include "Password can't be blank", "Password is invalid. Input half-width characters.", "Password confirmation doesn't match Password"
    end
    ...
    it "13お名前(全角)は、名前がそれぞれ必須であること" do
      @user.firstname = nil
      @user.valid?
      expect(@user.errors.full_messages).to include("Firstname can't be blank", "Firstname is invalid. Input full-width characters.")
    end
